...
- **生成/仲裁分工**:主 agent 負責仲裁 —— 組織任務、提煉 brief、挑戰每一個 subagent 的結論 —— 所有長文生成(plan、文件、程式碼、review)由 opus 的 persistent subagent session 承擔,執行者用 low effort、判斷角色跟主 session 同 effort:品質靠對抗式挑戰把關,token 省在大量寫作上
