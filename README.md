...
- **Generator/Arbiter Split**: the main agent arbitrates — organizes the task, distills briefs, challenges every subagent conclusion — while persistent opus subagent sessions carry all long-form generation (plans, docs, code, reviews), executors at low effort and judgment roles at the session's: quality held by adversarial challenge, tokens saved on bulk writing
