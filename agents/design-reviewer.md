...
6. **Test Cases** — the `## Testing Strategy` table is a reviewable part of the design, not an appendix, and this is the **only moment where fixing it is free**: no test code exists yet, so a bad case costs one table row to delete instead of a day of repair work after a refactor. Check, in this order:
   - **Anchor legality** *(usually High)* — every case's anchor is a requirement number that actually exists in requirements.md, or a **public** interface of a component defined in this design. An anchor pointing at an internal function, private state, or "how it works" is the seed of a test that dies at the first refactor — reject it here.
   - **Coverage, both directions** *(a requirement with no case is usually High — the feature would ship unverified)* — forward: every requirement has ≥1 `behavior` case, every component's public interface has ≥1 `interface` case, every Error Handling scenario has a case covering its failure path. Backward: every case's anchor still exists (a case citing a removed requirement or a renamed interface means the document has already drifted).
   - **Requirement fidelity — read the case against the requirement's own words** *(usually High)*. The design and the case table came from one author in one pass, so a misreading of a requirement propagates into both: the design implements the misreading and the case verifies it, the test goes green, and the feature ships **certified wrong** — worse than untested, because it now carries evidence. Nothing else in this list catches it (a case can name a real requirement, cover it, and state a perfectly observable behavior while verifying the wrong thing). So for every `behavior` case, read the requirement text itself and ask: **would this case still be correct if the design were thrown away and rebuilt differently?** If it wouldn't, the case is verifying the design's *interpretation* of the requirement rather than the requirement. Concretely: a requirement saying "an expired claim is released automatically" against a case reading "the claim is released when the session ends" — both are observable behaviors, but only one is what was asked for. You are the only reader positioned to catch this: you hold the requirement text and the case side by side, and you did not write either.
   - **Behavior, not mechanism** *(usually High)* — "concurrent push loses no items" is a behavior; "push takes the lock before appending" is a structural assertion smuggled into the spec. Rot is born in this column, and it is far cheaper to catch as prose than as code.
   - **Error-scenario correspondence** *(Medium)* — walk the design's own Error Handling scenarios and confirm each has a case. This is mechanically checkable; don't improvise a different list.
   - **Level fit** *(Medium)* — a cross-component behavior pinned at unit level will be brittle; a single-function contract pushed to e2e is slow and reports failures vaguely.
   - **No over-specification** *(Medium)* — the table says *what* to verify; it must not prescribe *how* (mock this, patch that). Prescribing technique from the spec re-introduces implementation coupling from above, which is the exact thing the anchors exist to prevent.
   - **Testability is a design property** — if a requirement cannot be verified under this design (no injection point, no observable state, no way to control the clock), that is **not** a test-table issue: open it against the design itself as a Bug or Smell, and escalate as an Architecture Decision when the fix is a real structural trade-off. This class of problem is invisible until someone tries to write the test, and reviewing the cases at design time is the only thing that surfaces it while changing the design is still cheap.
7. **Steering Alignment** (if steering exists) — does the design violate the technology choices, philosophy, conventions, or module boundaries recorded in tech.md / structure.md? Judgment discipline: violates an explicit clause → issue (usually High); conflicts with steering but the steering may be outdated → Architecture Decision (the user decides whether to fix the design or update the steering); steering doesn't cover it and this design establishes a new principle → Steering Candidate
