...
Both are the same instinct — *don't let the main agent commit a consequential call entirely on its own when a stronger model can weigh in first.* Application 1 is about not interrupting the user needlessly; application 2 is about not letting the main agent's solo arbitration of subagent output go unchecked.

> **The advisor is a main-agent-only tool — subagents never call it.** `spec-author`, `spec-implementer`, and the two reviewers do their bounded work and **escalate uncertainty up to the main agent** exactly as today (the reviewer hands its issue list up, `spec-author` flags an assumption rather than resolving it, `spec-implementer` reports a problem rather than assuming) — the main agent is the *single* point that consults the advisor and the user. Two reasons: (1) **economy** — the advisor is the most premium tier; letting subagents call it inverts the whole generator/arbiter cost model (the point of the split is that only the arbiter spends the top tier); (2) **the advisor's value is the whole picture** — it sees the *caller's* transcript, and only the main agent's transcript holds the full session; a subagent calling it would forward only its own narrow, bounded context. If a subagent finds itself wanting a stronger opinion, that is precisely the signal to escalate to the main agent, which then decides whether to consult the advisor.

**This rule cannot be enforced structurally — it rides entirely on the prohibition prose in the agent definitions.** That is a deliberate correction of an earlier assumption, and the reason matters, because the failure was silent:

- **`disallowedTools: advisor` does not bind.** Every one of this plugin's agents carries it in frontmatter, and the agent registry duly reports them as "all tools except advisor" — but the advisor is a **server-side tool**: it is attached to the request by the server when advisor mode is on, and never passes through the client tool registry that `disallowedTools` filters. It therefore remains callable, and in practice was called — across every one of the seven agent types, with real results coming back. The frontmatter is kept because it records the intent and would take effect if the tool ever became client-side, but it must never be cited as the enforcement.
- **A `PreToolUse` hook cannot intercept it either**, for the same reason: hooks fire on client tool calls, and a server tool never round-trips to the client. There is no interception point to hook.
- **So every agent definition carries an explicit "Never call the advisor" section** — all seven of ours, and a matching line in the **spawn prompt** for the built-in `Explore` / `general-purpose` agents the plugin fans searches out to (those we can't edit at all; see SKILL.md "Model Economy for Exploration"). The section has to do more than forbid: the harness injects advisor guidance that actively tells its reader to consult before substantive work, so the prohibition must name that guidance, say it is addressed to the main agent, and state that it is overridden — otherwise the agent is weighing an explicit instruction against silence.

Be honest about the strength of this: prose biases the outcome, it does not guarantee it. If subagent advisor calls keep appearing despite the prohibition, the remaining fix is upstream in the harness, not another mechanism inside this plugin.

> **Why this exists**: the main agent tends to stop and ask the user far more often than the work warrants — many of those stops are "which of these two technically-equivalent paths" questions that a competent engineer would just decide. Each stop costs the user a context-switch, and the reluctance to decide is itself expensive. The advisor is a cheaper second opinion that absorbs those stops, keeping the user's attention for the decisions that are actually theirs to make.
...
The same "consult the stronger model before committing a consequential call" applies **inside the review loop**, where the main agent judges the reviewer's returned output alone. The generator/arbiter split already has the main agent scrutinize the reviewer's list every round, challenging when the round warrants it (`review-protocol.md`, "The challenge exchange") — but the *outcome* of that scrutiny is still the main agent's solo call, and it's the call that decides what gets fixed versus dropped. At the consequential points, the advisor co-arbitrates:

- **When the main agent's arbitration would consequentially override the reviewer** — it's about to **dismiss a reviewer-graded Critical/High as a false positive**, **downgrade its severity** (e.g. Critical → Medium), or **accept essentially unchanged a list that carries Critical/High** (significant findings passing without pushback) — consult the advisor. A **waived trivial round** (all Medium/Low, accepted without a challenge per `review-protocol.md`'s waiver rule) is examined acceptance, not wholesale acceptance — no consult.
