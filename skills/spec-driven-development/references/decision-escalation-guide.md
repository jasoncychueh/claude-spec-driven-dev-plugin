...
## 兩拍制交付：先文字 briefing，再 AskUserQuestion

Decision 的 context **不放進** AskUserQuestion 的 question stem — stem 在 UI 是窄小的對話框，長 context 擠在裡面難讀，code preview 還會被折疊（「N lines hidden」）。交付固定兩拍：

1. **對話輸出 Decision briefing**（一般 markdown 文字 — 渲染完整、code 有 syntax highlight、留在 scrollback）：
   ...
   - Code 片段與同 codebase 對照組放這一拍
2. **接著 AskUserQuestion**：question stem 短（1-3 行命題，可寫「背景見上方說明」），options 維持 label + 多維度 description + `preview`

**這個規則適用本 skill 所有 AskUserQuestion 互動** — Architecture Decision 拍板、Medium/Low waiver 批次詢問、Steering Candidates 批次確認。Briefing 停點本來就是兩拍制，這裡是同一個 pattern 的一般化：**user 永遠先在對話裡看懂問題，才在對話框裡做選擇**。
...
| ✅ 做 | ❌ 不做 |
|---|---|
| 先文字輸出 Decision briefing（review 脈絡 + use case 走讀 + code），再發短 stem 的 AskUserQuestion | 把全部 context 塞進 question stem（UI 對話框難讀）；或更糟 — 不交代由來直接丟選項 |
...
**第二拍 — AskUserQuestion**（stem 短，context 已在上方）：
