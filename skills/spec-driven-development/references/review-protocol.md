...
- **One author session per cycle**: `spec-author` (design phase) and `spec-implementer` (implementation phase — **one session per parallel group** in Spec Mode, which is its normal multi-group shape; typically a single session in Quick Fix Mode, two when its change list fans out) likewise stay alive; fix dispatches resume the session that wrote the artifact — the fixer remembers why it wrote what it wrote. For Spec Mode implementation fixes, resume the Mode 1 session whose group owns the affected files; if ownership is unclear, spawn a fresh Mode 2 instance. `spec-tester` (Spec Mode) keeps its own session on the same terms — it is spawned alongside the implementer groups in Stage 1 and resumed for every test fix.
- **Name the asset, or spawn fresh.** Before every resume, state in one clause what that session's context gives *this* dispatch. Exactly two things qualify: **(a) tacit knowledge that exists nowhere on disk** — why the code is shaped the way it is, which approaches were tried and rejected, what the brief meant (this is why a fix resumes the session that wrote the code); or **(b) a corpus it has already read that this dispatch needs *again*** — the reviewer's artifact + steering + issue history across rounds, where re-acquisition costs more than staleness risks. **If you can't name one, spawn fresh.** Note what does *not* qualify: "it's the same feature", "it's still this cycle", "it's already warmed up". Those are topic continuity, and topic continuity is not an asset — a fresh agent reading the same files on disk is equivalent, cheaper in context, and unbiased. Prong (b) is the elastic one: it counts only when the new dispatch works over the **same** corpus, so a new phase's task group does not inherit the previous phase's reading. The named defaults elsewhere in this skill ("resume the group's owning session", "resume its Stage 1 session") are **instances that pass this test**, not exceptions to it — the owning session holds (a).
- **Retire a session at 400K of context, not at a round count.** Naming an asset per dispatch is the per-call test; this is the standing one. The measure is the executor's **context size**, because that is what actually drives both cost and defocus — an earlier version of this rule counted resumes, which was wrong in a way worth stating: bounded dispatches continued by resume are the prescribed shape, so a resume count rises precisely when the discipline is working. Measured across real sessions, executors reached a median peak of **461K** and ran as high as **997K**, and the subagent transcripts carry **151 `compact_boundary` markers** — proof that past this point the harness autocompacts the executor, which then keeps working from a lossy summary of its design basis instead of the basis itself, unable to tell what it lost. **400K is the ceiling and it is enforced by retirement, not by compaction**: it sits far below the ~930K the harness compacts at, so a session retired on time is never compacted at all. That ordering is the point — this workflow keeps the basis on disk, so a fresh spawn re-reads the authoritative design.md, tasks.md and review-log §1 (the same rebuild the resume-failure fallback uses) while a compacted session keeps only a paraphrase of them. Around **300K**, size the next dispatch to land under the ceiling rather than through it; at **400K**, retire unless the asset is still nameable and genuinely large. A `SendMessage` checkpoint hook reads the target session's current context and notes it at both marks — it only injects context, never blocks or prompts, because whether tacit knowledge is at stake is visible to you and not to it. The measurement is the part you cannot compute for yourself; the judgment is yours.
- **A dispatch is one bounded chunk of work, not a phase handed over in one go.** Size it deliberately: one task group, one review round, one settled approach — with an explicit deliverable — then resume for the next. Nothing truncates an over-large dispatch, so an unbounded one runs until it is done — a 285-turn implementer dispatch that misread the design burns all 285 turns before anyone sees a line of it. What guards against that is a **checkpoint that interrupts nothing**: once a dispatch passes the executor's threshold (45 tool calls for an implementer, 20 for a tester, and so on, repeating every threshold thereafter), a hook inside its own session asks it to `SendMessage` a short status to the arbiter and carry straight on working. The arbiter reads it on its next turn; if the direction is wrong it answers, and that answer reaches the executor mid-work. Nothing stops, nothing is truncated, and a dispatch going fine costs one tool call per checkpoint. See "Progress reports arrive mid-dispatch".
- **Mechanical work wants a fresh spawn, not a warm one.** Applying a stated fix to named files carries no tacit knowledge by definition — there is nothing to remember, and a long history is pure distraction on a mechanical dispatch. The named defaults ("resume the session that owns these files") assume the fix needs to know *why the code is shaped this way*; when it plainly doesn't, a clean executor is the more accurate one.
- **The asset decays; a long-lived session is not automatically the better dispatch.** The protocol already acts on this: the convergence fuse spawns a *fresh* reviewer at Round 5 precisely because a session that has been through many rounds becomes anchored on its own earlier conclusions. That is the general case, not a special rule for Round 5 — the longer a session runs, the more its context is a record of its own reasoning rather than the world. Weigh that against re-acquisition cost rather than resuming on reflex.
- **Session scope = one loop, and resume never crosses it**: "resume, don't respawn" is a rule *inside* a work cycle — the design loop, implementation loop, or quick fix the session was spawned for. It is **not** a standing default that the next task inherits the last session. A new spec, a new quick fix, or any task unrelated to what the session was spawned for gets a **fresh spawn** — always. The test before every resume: *does this dispatch continue the work this session already holds in context?* If yes (a fix round, a revision round, a follow-on artifact of the same spec), resume. If no, the old context is not an asset but a liability: a stale design basis and issue history from another task can silently steer the new work, and the accumulated context burns window for nothing. Cross-spec / cross-quick-fix resumes are never correct.
...
### The challenge exchange (rigor challenge — reviewer, when warranted)

What is mandatory every round is **scrutiny**: the main agent reads the artifact (round 1 in full, later rounds the diffs) and honestly judges the reviewer's list before acting on it. Sending a **challenge message** is the *escalation* of that scrutiny, not its ritual proof — at most one per round, and only when the round warrants it. **A manufactured challenge is a defect, not diligence**: inflating a wording nitpick into a dispute to satisfy the ceremony burns a full round-trip, trains the reviewer to defend trivia, and buries the signal the challenge exists to sharpen.

**Challenge required** — any one of these makes the round consequential enough that scrutiny must become dialogue:

1. The round's list contains any **Critical/High** (probe it: concrete scenario, severity honesty, what's adjacent that wasn't flagged)
2. The main agent **disagrees with anything** — a suspected false positive, a wrong severity, a suspected miss. The disagreement *is* the challenge content; acting on it silently is what the challenge exists to prevent
3. The round **declares convergence** (`0 issues` exit) after a loop that ever surfaced a Critical/High, or for a change of non-trivial scope — probe convergence honesty before accepting the exit
4. The list reads **miscalibrated against the change's risk** — an all-trivial list for a risky diff is itself the anomaly to challenge

**Challenge waivable** — only when *all* hold: every issue is Medium/Low **and** of the directly-verifiable kind (wording, naming, doc phrasing, an obvious local fix); the main agent has actually read the artifact and agrees with every item **as graded** (any intended dismissal or downgrade re-triggers requirement 2); and nothing smells miscalibrated. Then accept the list as final without a challenge message and record `accepted without challenge` in the round's log entry. Waiving is a conclusion reached *after* reading — never a default, and never a bar-lowering signal to the reviewer: review depth stays constant whether or not the arbiter ends up with something to say.

- At most one exchange per round, no inner loops — if a disagreement survives it, that is by definition a contested design choice: escalate it as an Architecture Decision, don't relitigate
- The challenge must engage with the artifact's substance; "are you sure?" is not a challenge
- **Consult the advisor before a consequential arbitration** (the advisor gate's second application — SKILL.md "Advisor Gate Mechanism" / `advisor-gate-guide.md`): the challenge is the main agent's own judgment of the reviewer's output, and its *outcome* decides what gets fixed vs dropped. When the main agent is about to **dismiss a reviewer-graded Critical/High as a false positive**, **downgrade its severity**, or **accept essentially unchanged a list that carries Critical/High**, consult the advisor **before sending the single challenge** so its view shapes that challenge. (A waived trivial round — all Medium/Low, accepted without challenge per the rule above — is *not* wholesale acceptance in this sense and triggers no advisor consult; the trigger guards unexamined acceptance of significant findings, and a waived round is examined by definition.) Consequential-or-uncertain trigger only (routine "confirm a Critical is real and fix it" needs no advisor); a surviving advisor dissent that implies deeper probing escalates as an Architecture Decision (not a second challenge); degrades to solo arbitration when the advisor is unavailable; the advisor's concurrence/dissent is annotated in the review log (§4 `(advisor concurred, DATE)` for a confirmed dismissal, a §1 severity-change note for a concurred downgrade, a kept-outcome §1 note on dissent)
...
### A round is one completed issue list — not one dispatch

**The unit of the protocol is the finished issue list, not the message that asked for it.** A single round may take several dispatches, and that changes nothing about the count.

A dispatch can come back **incomplete** — the agent reports having covered only part of what it was given (and, if the harness cut it, marks the output partial with a note to continue via `SendMessage`). That is a normal event, not a failure and not a result.

When a reviewer dispatch comes back incomplete:

- **Resume it and let it finish.** The session holds everything it has read so far; continuing costs nothing but the message.
- **The round number does not advance.** `D3` is still `D3` however many dispatches it took to produce D3's list.
- **Write no §1 audit-trail row.** A row records a completed round; there is nothing to record yet.
- **An incomplete return is never `0 issues`.** An incomplete sweep that has not yet found a problem looks exactly like a clean one. Convergence may only be declared from a round the reviewer itself concluded — the `Conclusion` section is what makes a list a round.

The same rule holds for every other agent: an author's document, an implementer's task group, and a tester's case set are each done when the agent says they are done, not when a dispatch returns. **A truncated dispatch is never a finished unit of work.** The count follows the concluded list, never the message boundary.

### Progress reports arrive mid-dispatch — read them, answer only if the direction is wrong

Executors send the arbiter a status line while they are still working. A checkpoint hook inside each executor's session asks for one every threshold of tool calls (implementer 45, implementation-reviewer 40, design-reviewer and researcher 30, author 25, tester and verifier 20, tasks-design-verifier 15), and the executor sends it and carries on without pausing. It arrives in the main agent's context on its next turn, prefixed by the harness as a message from a subagent.

**Most of them need no answer.** A status that matches what was dispatched is confirmation that the dispatch is running true; reading it and saying nothing is the correct handling. Silence costs nothing and the executor is not waiting on a reply.

**Answer when the status shows the work going somewhere the dispatch did not ask for**: a file being edited that the task group does not name, an approach settled that the design basis does not carry, a requirement being re-interpreted rather than applied, a fact reported that invalidates the dispatch. A `SendMessage` back reaches the executor between turns, mid-work — it does not have to finish first — so a correction sent now saves the rest of the dispatch rather than the next one.

**What the answer may say**: narrow the target (this file, this function, this requirement section), correct a misread of the design, supply a fact the executor could not verify, or stop a line of work and name the one to resume. **What it may never say**: read less. An executor that reports having spent the dispatch reading is reporting the part of the task that comes first; if that reading is going the wrong way, name what to read instead — never take the basis away while keeping the output demand. See below.

**A progress report is not a result.** It does not conclude a round, it advances no number, and it earns no §1 audit-trail row. The dispatch is still running.

### How to read an incomplete return — reading is work, not a stall

An incomplete return says one thing: the dispatch was larger than what the executor got through. It does not say the executor was idle, and it does not say it was reading instead of working. The harness note that can accompany it — "was still calling tools and had produced no report" — describes every executor cut mid-task, including one doing exactly what it should.
...
**What a real stall looks like** — and only this justifies treating the session as stuck: across two consecutive returns from the same session, the executor re-reads the same files it read in the previous dispatch, or nothing on disk has changed and its report names no new fact. One incomplete return is never evidence of a stall; it is evidence of a dispatch bigger than one pass. A session that is stuck is retired and respawned fresh on a narrower target, exactly as at 400K — even then the fix is a smaller question, not a reading ban.

**The resume, therefore, is one of two things.** "Continue" — the default, and correct far more often than it feels. Or a **narrower target**: which file, which function, which requirement section — scope small enough to reach an edit in one pass. Never a reading ban as a response to an incomplete return, and never an instruction to output on facts the executor has not verified. (Reading bans that exist for another reason stand — `spec-tester` Mode 1 must not read the files the implementer is writing, which is a coupling rule, not a budget response.)
...
6. **Pin the tier when you fan out a search**: if a spot-check makes you delegate a broad codebase sweep to a built-in `Explore` / `general-purpose` agent, pin its model instead of inheriting yours — `model: haiku` for mechanical search (locate a file, find a symbol, enumerate callers), `model: opus` when the search must reason across files; cap at opus, never leave it to inherit. For a known target, read it directly (`Grep` / `Read`) rather than spawning an agent. Why? A broad read is bulk work priced by volume, not judgment — running it on the top tier by default burns tokens the review doesn't need. And add a line to that spawned agent's prompt telling it **not to use the advisor tool** — nothing removes the advisor from a subagent's reach (it is served from outside the client tool registry, so no frontmatter field and no hook can strip it), which makes the spawn prompt the only place to keep the most-premium tier out of a cheap bulk search
7. **Never call the advisor yourself**: it is available to you and the guidance attached to it is addressed to the **main agent**, not to you — that guidance does not transfer to a subagent, however strongly it is worded. Wanting a stronger opinion on a call is the signal to **hand the question up in your issue list** (an Architecture Decision is exactly that shape), not to consult. Why? A cheaper-tier executor spending the top tier inverts the generator/arbiter economy, and the advisor only sees the caller's transcript — yours holds your narrow slice of the session, so the opinion would come back confident and under-informed. The main agent is the single point that consults the advisor, and it will route your Decision through that gate for you. **If you are the main agent reading this list, this item is not yours** — these are the *reviewers'* disciplines; the advisor is your tool and the Advisor Gate tells you when to use it
...
1. **Challenge, then dispatch**: after each round's issue list, run the challenge exchange (see "Persistent sessions and the challenge exchange"), then hand the **final post-challenge list** to the corresponding fixer
   ...
   - implementation stage (both modes): **triage each fix by rung before dispatching** (SKILL.md "The fix escalation ladder") — mechanical items with an explicit landing point go to `spec-implementer (Mode 2)` (Spec Mode prefers resuming the session whose group owns the affected files; Quick Fix Mode resumes its implementer session); items whose **root cause is unknown, that admit several viable directions, or that touch framework code or a cross-component contract** go to `spec-author` first, which investigates and settles the approach into the design basis, after which the implementer implements it. **Issues against the test suite: in Spec Mode they go to `spec-tester` (Mode 2)** — resume its Stage 1 session — never to the implementer, since a test and a code fix stay separate dispatches even when one issue produced both. **In Quick Fix Mode there is no tester**: the implementer wrote the tests and fixes them, in the same Mode 2 dispatch as the code. The main agent doesn't write code directly in any path
