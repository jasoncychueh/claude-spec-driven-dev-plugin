...
- **Retire a session that has outlived its asset.** Naming an asset per dispatch is the per-call test; this is the standing one. An executor session accumulates a round of its own reasoning every resume, and past roughly **15 rounds** its context is dominated by that history rather than by the task in front of it — a cheaper-tier executor then loses the thread of what it is currently supposed to do, which reads as carelessness but is a context problem. Measured across real sessions: the median executor takes ~10-15 resumes, and the tail reaches **119**. So around 15 rounds, check the asset deliberately rather than by reflex; by **25**, retire the session unless the asset is still nameable and genuinely large — hand a fresh spawn the current artifact state plus review-log §1 (the same rebuild the resume-failure fallback already uses) and continue. A `SendMessage` checkpoint hook notes the count at 15 and again from 25 — it only injects context, never blocks or prompts, because whether tacit knowledge is at stake is visible to you and not to it. The count is the part you cannot compute for yourself; the judgment is yours.
