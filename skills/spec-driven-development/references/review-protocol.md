...
- **A dispatch is one bounded chunk of work, not a phase handed over in one go.** Every executor carries a `maxTurns` ceiling in its frontmatter, so an over-large dispatch is cut and returned **partial** rather than run to exhaustion. Size dispatches to that shape deliberately instead of discovering it: one task group, one review round, one settled approach — with an explicit deliverable — then resume for the next. This is not only a context measure (a single dispatch adds only ~20-64K); it is what gives the arbiter a look at real work in progress. A 285-turn implementer dispatch that misread the design burns all 285 turns before anyone can see it. **A partial return is not a result** — resume it; see "A round is one completed issue list".
...
This matters because executor agents carry a `maxTurns` ceiling (their frontmatter sets it), so a dispatch that runs long is **cut and returned marked partial**, with the harness's own note that the output is incomplete and should be continued via `SendMessage`. That is a normal, expected event — not a failure, and not a result.

When a reviewer dispatch comes back partial:
...
- **A partial return is never `0 issues`.** An incomplete sweep that has not yet found a problem looks exactly like a clean one. Convergence may only be declared from a round the reviewer itself concluded — the `Conclusion` section is what makes a list a round.
