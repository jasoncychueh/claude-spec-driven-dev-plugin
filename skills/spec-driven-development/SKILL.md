...
| 2. Implementer report | `spec-implementer`'s completion report (both modes) | out-of-scope findings — pre-existing bugs, adjacent tech debt; the implementer reports, the main agent walks the triage ladder (fix in passing / absorb / backlog) and records only the honest residual |
