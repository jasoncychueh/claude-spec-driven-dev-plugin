...
**Every dispatch is bounded by the agent's own `maxTurns` ceiling.** Handing over more than one bounded chunk does not make the executor do more — it makes the dispatch come back **partial**, cut mid-work. So size the chunk deliberately: one task group, one review round, one settled approach, each with an explicit deliverable, then resume for the next. The measured shape this replaces is a 285-turn implementer dispatch that, had it misread the design, would have burned all 285 turns before the arbiter saw a line of it. **A partial return is not a result** — resume it, and never read it as a finished list or a `0 issues` round (`review-protocol.md`, "A round is one completed issue list"). **And a partial return is not a stall**: an executor cut while still reading was doing the part of the task that comes first — reading the design basis and the target files *is* the work. Resume with "continue" or a narrower target (which file, which function); never with a reading ban in response to the partial, nor an instruction to write on facts it has not verified, which is spec drift by dispatch prompt (`spec-tester` Mode 1's ban on reading the implementer's files is a coupling rule and stands). Reordering or scoping the reading is fine; removing the basis while keeping the output demand is not, and if reading must be cut short the executor lists what it did not verify rather than guessing. A stall is two consecutive partials with the same files re-read or no state change on disk — one partial is evidence of a budget, nothing more (`review-protocol.md`, "How to read a partial return").
