...
- **`pick <id>`**: open the item file, brief the user on Problem / Context / Suggested next step (use-case-driven, per `briefing-guide.md` — assume they've forgotten the original discussion). If the item is already `[~]` (claimed), stop and report the claim — since when, which branch, what's being done — and let the user decide (pick something else / take it over / coordinate); never silently take it over. After the user confirms, **claim it immediately — before any planning begins, not at implementation start**: frontmatter `status: in-progress` + `picked_up: DATE (branch: X) — one sentence on what's being done` (branch not created yet → write the intended name or `TBD`, update later), and the index line flips to `[~]` carrying the same claim inline (`backlog-guide.md` → "Claiming an item"). Plan Mode / spec authoring are already "the work" — an item left `[ ]` through the planning phase is the exact window where a parallel session picks it too. Then **route it like any incoming task**: decide Quick Fix Mode vs Spec Mode per `mode-selection.md` and run the normal flow — the item file's content seeds the brief. When the work completes, close the item as `done`.
...
**Claiming (the other race)**: unique ids stop two sessions recording the same id; they don't stop two sessions working the same item. The claim is written **at pick confirmation, before any planning begins** — Plan Mode / spec authoring already count as the work, and an item left unclaimed through planning is the exact collision window: the item is marked `status: in-progress` + `picked_up: DATE (branch: X) — what's being done`, and the index line flips to `[~]` carrying the same claim inline. **Every entry path claims, not just `/backlog pick`** — a task that arrives any other way but covers an open item claims it at that same moment. On encountering a `[~]` item, never silently take it over or skip it — report the claim (since when / which branch / what) and let the user decide. No expiry rule: whether an old claim is dead or a live long-running branch isn't knowable from inside the repo.
...
**The advisor is a main-agent-only tool.** The main agent is the single point that consults the advisor (and the user); subagents escalate uncertainty *up to the main agent* as they do today. This is enforced structurally, two ways: this plugin's own agents each set `disallowedTools: advisor` in frontmatter (the tool is removed from their pool; all other tools, including MCP, stay), and a spawned built-in `Explore` / `general-purpose` agent — which can't be frontmatter-restricted — is told not to use the advisor in its spawn prompt (see "Model Economy for Exploration"). The reason: letting a cheaper-tier executor call the most-premium tier would invert the generator/arbiter economy, and the advisor's whole-session value is only realized from the main agent's transcript anyway.
...
**Tell a spawned `Explore` / `general-purpose` agent not to use the advisor.** These are *built-in* agents — we can't set `disallowedTools: advisor` on their frontmatter the way the plugin's own agents do, and the Task tool has no per-call tool-restriction parameter, so the **only** lever is the spawn prompt. Add a line like *"Do not use the advisor tool — that is the main agent's, not yours; just return your findings."* Same reasoning as the main-agent-only rule (see "Advisor Gate Mechanism"): a cheap-tier bulk search must not reach for the most-premium tier.
...
## Dispatch Granularity (how much goal per dispatch)

The generator/arbiter split says *who* executes; this section says **how much to hand an executor per dispatch**. Two shapes, chosen by how clear the path is — not by task size:
...
| `references/advisor-gate-guide.md` | Advisor Gate — consult the advisor before a consequential call. App 1 (escalation gate): three-tier ladder, triage, graceful degradation, recording advisor-resolved decisions, surfacing at the briefing. App 2 (co-arbitrating reviewer output): the challenge-exchange / convergence consult + recording. Plus the main-agent-only rule (subagent `disallowedTools` + built-in-agent spawn-prompt) |
