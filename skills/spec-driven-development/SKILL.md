...
**Execution architecture (who generates, who arbitrates)**: the main agent — running the session's model at the session's effort — acts as orchestrator, arbiter, and the user's single interlocutor: it discusses requirements, distills briefs, challenges subagent conclusions, escalates decisions, and maintains the review log. **All long-form generation lives in persistent subagent sessions**, all on opus: the executors (`spec-implementer`, `spec-tester`, the two verifiers) pinned at **low** effort, the judgment roles (`spec-author`, the two reviewers) at **the session's own effort** — so the user sets how hard the judgment layer thinks with `/effort`, according to the budget left: `spec-author` writes plans and spec documents, `spec-implementer` writes code (both modes), `spec-tester` writes the tests in parallel with it and blind to it (Spec Mode), the two reviewers review — each session spawned once per loop and **resumed via SendMessage across rounds** instead of respawned, so nothing gets re-read that's already in a session's context. Resume is justified by a **named asset** (tacit knowledge, or a corpus this dispatch re-reads), not by the session merely existing — no asset, fresh spawn. Resume is scoped to that loop: a new spec, a new quick fix, or an unrelated task always spawns fresh — never inherit another task's session (see `review-protocol.md` "Persistent sessions"). The arbiter's per-round scrutiny — reading the artifact and challenging when the round warrants it — is what keeps the review deep at whatever effort it runs (mechanics and the required-vs-waivable rule in `review-protocol.md`, "Persistent sessions and the challenge exchange"). The main agent never writes production code or long-form documents in either mode.
...
3. Launch `spec-implementer` agents (Mode 1) — spawn every group **in parallel within the same message** (separate messages serialize them). **A new phase's groups are fresh spawns**: grouping is recomputed per phase, so a new group is new work over files the old session never read — resuming one there inherits an unrelated reading list and the tacit knowledge it holds is about other code (see `review-protocol.md`, "Name the asset"); each agent receives the feature name, its task list, the `Mode 1` marker, and reads design.md via the `Design ref` field. **Keep each group's session resumable** — Stage 2 fix dispatches prefer resuming the session that owns the affected files
...
### /roadmap [show \<id\> | next | promote \<id\> … | walk \<id\> | any edit]
...
- **Editing the tree** — every change has a command; hand-editing `roadmap.json` is never the way around a missing one (the script validates the whole board before it saves, the hand edit does not):
  - structure: `roadmap add <parent> <id> <title>`, `roadmap move <id> <newParent> [--before <sibling>]` (re-parent or reorder; the root is `root`), `roadmap rename <id> <newId>` (dependencies follow), `roadmap remove <id> [--force]` (unfinished branches only)
  - fields: `roadmap set <id> <field> <value>` — status, ordinal, title, summary, detail, spec, dependsOn, updated; `-` clears. History: `roadmap note <id> <text>`. The root: `roadmap root title|intro|now …`
  - tickets: `promote <ticket> --into <node>` attaches, `demote <ticket>` returns it to the basket — demote then promote to move it between nodes
  - a closed ticket's record: `backlog set <id> resolution <text>` (e.g. a commit hash that changed in a rebase)
- **Starting and finishing work**: when an item starts, `roadmap set <id> status active` and route it like any task (Quick Fix Mode / Spec Mode per `mode-selection.md`, its body as the seed; `set <id> spec <feature>` once a spec exists). When it finishes, `roadmap finish <id> --resolution "<where it landed>"` — it refuses while any design question or child item under it is unfinished, folds the settled question nodes away (their conclusions are already in the body), and archives the body; the node stays as part of the architecture. `show` folds finished parts to one line (`--done` expands). Update the root's `now` paragraph whenever the focus changes: `roadmap root now "<paragraph>" …` (`title` and `intro` likewise) — never edit `roadmap.json` by hand.
...
- **Persistent sessions**: one reviewer session per loop, spawned at Round 1 and **resumed via SendMessage** each later round (no respawning, no re-reading); `spec-author` / `spec-implementer` / `spec-tester` sessions likewise stay alive for fix dispatches. **Scope = that one loop/cycle**: resume only a dispatch that continues the work the session already holds — a new spec, a new quick fix, or an unrelated task always spawns fresh (another task's stale context is a liability, not an asset). **Within a cycle, resume is still justified per dispatch, not assumed**: name in one clause what that session's context gives *this* dispatch — tacit knowledge that lives nowhere on disk (why the code is shaped this way, what was tried and rejected), or a corpus it has already read that this dispatch needs again. Can't name one → spawn fresh. **Mechanical work (apply this stated fix to these named files) has no tacit knowledge by definition** — a clean executor is the more accurate one there, not the warm one. "Same feature" and "already warmed up" are topic continuity, not an asset; a fresh agent reading the same files is equivalent and unbiased. The asset also decays — the convergence fuse spawns a *fresh* reviewer at Round 5 exactly because a long-running session anchors on its own earlier conclusions (`review-protocol.md`). **Sessions are retired at 400K of context, not run indefinitely**: the measure is context size, not resume count — bounded dispatches continued by resume are the prescribed shape, so resume counts rise when the discipline is working. Measured executors peaked at a median of **461K**, reached **997K**, and the subagent transcripts hold **151 `compact_boundary` markers**: past that point the harness autocompacts the executor, which then works from a lossy summary of its design basis instead of the basis itself, unable to tell what it lost. 400K is enforced by **retirement, not compaction** — it sits far below the ~930K compaction point, so a session retired on time is never compacted; a fresh spawn re-reads the authoritative design.md / tasks.md / review-log §1 while a compacted one keeps only a paraphrase. Around 300K, size the next dispatch to land under the ceiling; at 400K, retire unless the asset is still nameable and large. A `SendMessage` hook reads the target's current context and notes it at both marks — context only, never a block or a prompt: it supplies the measurement you can't compute for yourself, and the decision stays yours. **Dispatch in bounded chunks**: nothing truncates an over-large dispatch — a checkpoint hook has each executor send you a status every threshold of tool calls while it keeps working, and you answer only if the direction is wrong, but that is visibility, not sizing — so size dispatches deliberately (one task group, one review round, one settled approach, each with an explicit deliverable) and resume for the next. **An incomplete return is never a finished result** and never `0 issues`: resume it, don't advance the round number, and write no §1 row until the reviewer concludes the round itself (`review-protocol.md`, "A round is one completed issue list"); and it is not a stall either — an executor that spent the dispatch reading was doing the work, so resume with "continue" or a narrower target, never a reading ban (`review-protocol.md`, "How to read an incomplete return"). Resume fails → fresh spawn rebuilt from review-log §1 (see `review-protocol.md`)
...
**The advisor is a main-agent-only tool.** The main agent is the single point that consults the advisor (and the user); subagents escalate uncertainty *up to the main agent* as they do today. The reason: letting a subagent call the most-premium tier would invert the generator/arbiter economy, and the advisor's whole-session value is only realized from the main agent's transcript anyway.
...
The generator/arbiter split (Principle #10) moves long-form **generation** into subagents at a deliberate effort level. The same reasoning applies to **reading**: broad codebase exploration — sweeping many files to locate something, map a subsystem, or trace callers before distilling a brief — is bulk work whose cost is driven by how much gets read, not by how much judgment it needs. When the main agent (or any subagent) fans that out to a built-in `Explore` / `general-purpose` agent, that agent **inherits the session model by default** — which may be the most expensive model and effort the account runs. A broad sweep at that tier burns tokens out of all proportion to the judgment it actually requires.
...
**Every dispatch is bounded by the arbiter, and watched by a checkpoint.** Nothing truncates an over-large dispatch — hand over a whole phase and it runs until it is done, however far it has drifted. What catches that is a hook inside the executor's own session: past a per-agent threshold it asks the executor to `SendMessage` a status to you and keep working, repeating every threshold. Those reports land in your context on your next turn; answer only when the direction is wrong, and the answer reaches the executor mid-work (`review-protocol.md`, "Progress reports arrive mid-dispatch"). That is visibility, not a substitute for sizing — it tells you a dispatch has drifted, it does not make an over-large one safe. So size the chunk deliberately: one task group, one review round, one settled approach, each with an explicit deliverable, then resume for the next. The measured shape this avoids is a 285-turn implementer dispatch that, had it misread the design, would have burned all 285 turns before the arbiter saw a line of it. **An incomplete return is not a result** — resume it, and never read it as a finished list or a `0 issues` round (`review-protocol.md`, "A round is one completed issue list"). **And it is not a stall**: an executor that spent the dispatch reading was doing the part of the task that comes first — reading the design basis and the target files *is* the work. Resume with "continue" or a narrower target (which file, which function); never with a reading ban, nor an instruction to write on facts it has not verified, which is spec drift by dispatch prompt (`spec-tester` Mode 1's ban on reading the implementer's files is a coupling rule and stands). Reordering or scoping the reading is fine; removing the basis while keeping the output demand is not, and if reading must be cut short the executor lists what it did not verify rather than guessing. A stall is two consecutive returns with the same files re-read or no state change on disk — one incomplete return means only that the dispatch was bigger than one pass (`review-protocol.md`, "How to read an incomplete return").

**Fan out wide, in one message.** Groups whose `File:` sets are disjoint run concurrently only when spawned **in the same message** — separate messages serialize them, which is the difference between a phase finishing in one wall-clock block and finishing in four. Measured across real cycles, the work ran overwhelmingly phase-by-phase and serial: the parallelism this skill already specifies was mostly left unused. Be honest about the trade it buys, though — the same work compressed into fewer wall-clock hours draws harder on a rolling usage window, so wide fan-out trades rate-limit headroom for elapsed time. That is usually the right trade, and it is a trade, not a free win.
...
> **Say that the list is the turn's work, not a preview of it.** A batch handed over as a list is often executed one item deep and then reported on — the rest read as context rather than instructions, which turns a batch into an accidental stepwise dispatch that costs a main-agent round trip per item. Close it in the prompt: *"Work the list to exhaustion in this turn — finish an item, start the next, and report only when the whole list is done or you hit a blocker."* That last clause matters: without it, "don't stop" collides with the stop-instead-of-thrash rule, and a genuinely blocked executor will grind rather than report.
...
In stepwise mode the main agent does the pathfinding **itself, before dispatching** — that is arbiter judgment, not executable legwork, so it belongs with the arbiter. Think through the concrete route, break it into the smallest independently verifiable goals (one file's change, one failing test made green, one interface stubbed), then dispatch **one goal at a time** to the *same resumed executor session*; verify the outcome, and only then issue the next. Give each step stronger guidance than a batch dispatch would carry — expected outcome, the specific files, what "done" looks like, known traps. The executor runs at low effort: when the path is unclear, **precision of instruction substitutes for depth of thought**.
...
**The asymmetry the executors can't see — they cannot be woken.** The main agent may hand work to the background and end its turn: the harness wakes it with a completion notification. **A subagent has no such mechanism.** It is a transcript, not a live process — the instant it ends its turn it stops completely, and the only thing that can ever restart it is the main agent sending it a message. A command it starts with `run_in_background` genuinely keeps running and genuinely finishes; nothing then exists to tell it. Nothing in the tools signals that the two cases differ, so the executors are told explicitly (in `spec-implementer.md` / `spec-author.md`): **foreground everything, narrow what is too slow, escalate what can't be narrowed.** **The executors are actively told the opposite by their own tool guidance**, which is why prose in their definitions was not enough on its own: the harness tells its reader that a backgrounded task notifies on completion and that polling is therefore wasteful. That text is written for *you*; it reaches them as injected boilerplate and is false about them. Observed failure: an executor backgrounds `dotnet test`, ends its turn "waiting for the notification per the no-polling discipline", and the dispatch stalls until a human notices — repeatedly, in one session, even with the rule spelled out in the dispatch prompt. Their definitions now name that guidance and override it, and tell them that a task *already* backgrounded must be polled to completion **within the turn**.

The main agent's side of this is twofold:

- **Scope or absorb the long runs.** When a dispatch will need a genuinely long build or test suite, either bound the executor's run, or run the command yourself and hand the result into the session — you have the wake-up they lack.
- **Every executor dispatch prompt carries the rule.** One line, always: *"Nothing wakes you — no `run_in_background`. Run long commands in the foreground with the timeout at maximum, or poll to completion inside your turn; never end a turn waiting for a completion notification."* Redundant with their definitions by design: this is the failure that costs the most when it slips, since it stalls the whole line rather than degrading one result.
- **Recognize the symptom instantly.** An executor report containing "waiting for the background task to complete" is a **stalled dispatch, not a status update**. Resume it immediately with the instruction to poll in-turn; don't wait, and don't treat the promised follow-up as something that will arrive.
...
| 3 | **Root cause unknown**, or **multiple viable directions**, or the fix **touches framework / a cross-component contract** | **`spec-author`** (judgment tier) investigates and settles the approach into the design basis; the implementer then implements the settled approach |
...
10. **Generation in Subagents, Arbitration in the Main Agent** — the main agent converses, distills briefs, challenges, escalates, and keeps the review log; long-form generation (plans, spec docs, code, reviews) runs in **persistent subagent sessions** on opus — executors at low effort, judgment roles at the session's effort — resumed across rounds via SendMessage. Per-round scrutiny — with a challenge when the round warrants one — is what keeps the generation trustworthy (see `review-protocol.md`). The same tier discipline extends to **reading**: broad codebase exploration is delegated to a built-in agent on a **pinned** cheaper tier (haiku for mechanical search, opus at most), never left to inherit the session's top model — see "Model Economy for Exploration". And **dispatch granularity tracks path clarity**: a clear design basis → batch dispatch; thrashing or an unsettled basis → the arbiter decomposes the goal itself and dispatches small verified steps one at a time, while executors stop-and-escalate instead of thrashing. The same economy governs **who settles a fix**: mechanical fixes go straight to the implementer, but diagnosis (unknown root cause / several viable directions / framework or contract impact) escalates to `spec-author`, and a problem the judgment layer already failed on escalates again to the main agent's own diagnosis — asking the execution tier to do the judgment part is the inversion this principle exists to prevent — see "Dispatch Granularity"
