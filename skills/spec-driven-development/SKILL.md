...
In stepwise mode the main agent does the pathfinding **itself, before dispatching** — that is arbiter judgment, not executable legwork, so it belongs with the arbiter. Think through the concrete route, break it into the smallest independently verifiable goals (one file's change, one failing test made green, one interface stubbed), then dispatch **one goal at a time** to the *same resumed executor session*; verify the outcome, and only then issue the next. Give each step stronger guidance than a batch dispatch would carry — expected outcome, the specific files, what "done" looks like, known traps. The executor runs at medium effort: when the path is unclear, **precision of instruction substitutes for depth of thought**.
